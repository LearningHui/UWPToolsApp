...
//  Created by zou.penghui on 2016/6/28
...
namespace LuaScript
{
    public class LuaHttp
    {
        ...
        static int postSyn(int L)
        {            
            Task<string> task = HttpRequestSync("http://baike.baidu.com/cms/global/lemma_config.json");
        ...
        static Task<string> HttpRequestSync(string url)
        {
            ...
            AsyncCallback getResponseCallBack = (ar) => {
                try
                {
                    ...
                    {
                        //未获取到请求结果
                        System.Diagnostics.Debug.WriteLine("can't get response data:/r/n" + url);
                    }
                }
                catch
                {
                    System.Diagnostics.Debug.WriteLine("web request error!/r/n" + url); 
                }
                finally
                {
